...
barplot(total,
        main = "Number of Males and Females", xlab = "Gender", ylab = "Count", col = c("black", "violet"))
...
The plot shows a visual representation of the Sepal Length and Sepal Width for each Iris flower species. Each species is represented by a different color and shape.
...
Tahis plot allows you to easily compare and differentiate between different species based on their sepal length and sepal width, offering a comprehensive and visually appealing representation of the data.
...
print(alexa_file)  
write.csv(alexa_file, file = "/cloud/project/Worksheet#4/alexa_file_modified.csv", row.names = FALSE)
```
...
# Modify the 'variation' column using %>% and str_replace
alexa_file$variation <- alexa_file$variation %>%
  str_replace("Black Dot", "Salinas") %>%
  str_replace("White Dot", "Reysha Marie")

# Print the modified data frame
print(alexa_file)

# Read the modified CSV file
alexa_file_modified <- read.csv("/cloud/project/Worksheet#4/alexa_file_modified.csv")


knitr::include_graphics("/cloud/project/Worksheet#4/output.png")
...
barplot(height = variations$total,
...
# Read the dataset and load it into a data frame
df <- read.csv("alexa_file.xlsx")

# Define a variable "Black" for the black variation and another variable "White" for the white variation
Black <- df$Black
White <- df$White

# Create a barplot() for the black and white variations
BlackWhiteBarplot <- ggplot(df, aes(x = "Variations", y = value, fill = name)) +
 geom_bar(stat = "identity", position = position_dodge()) +
 labs(title = "Black and White Variations",
       x = "Variations",
       y = "Count",
       fill = "Variation") +
 theme_minimal() +
